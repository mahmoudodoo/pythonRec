...
with open(drive, "rb") as fileD:
    ...
    while byte:
        found = byte.find(b'WMV')
        if found >= 0:
            print(f'==== Found WMV at location: {hex(found + (size * offs))} ====')
            drec = True
            rcvd += 1

            # Extract the WMV segment
            wmv_segment = byte[found:]
            ...
            if start_date <= file_creation_date <= end_date:
                ...
                rec_directory_path = os.path.join(script_directory, current_rec_directory, f'{rcvd}.wmv')
                with open(rec_directory_path, 'wb') as fileN:
                    fileN.write(wmv_segment)
                    while drec:
                        byte = fileD.read(size)
                        if byte.startswith(b'WMV'):  # Check for the start of another WMV segment
                            fileN.write(byte)
                            print(f'==== Wrote WMV to location: {rec_directory_path} ====\n')
                            drec = False
                        else:
                            fileN.write(byte)
