...
end_date = datetime.datetime(2023, 8, 26)  # Specify the end date
...
with open(drive, "rb") as fileD:
    ...
    while byte:
        found = byte.find(b'PK\x03\x04')
        if found >= 0:
            print(f'==== Found ZIP at location: {hex(found + (size * offs))} ====')
            drec = True
            rcvd += 1

            # Extract the ZIP segment
            zip_segment = byte[found:]
            ...
            if start_date <= file_creation_date <= end_date:
                ...
                rec_directory_path = os.path.join(script_directory, current_rec_directory, f'{rcvd}.zip')
                with open(rec_directory_path, 'wb') as fileN:
                    fileN.write(zip_segment)
                    while drec:
                        byte = fileD.read(size)
                        if byte.startswith(b'PK\x03\x04'):  # Check for the start of another ZIP segment
                            fileN.write(byte)
                            print(f'==== Wrote ZIP to location: {rec_directory_path} ====\n')
                            drec = False
                        else:
                            fileN.write(byte)
