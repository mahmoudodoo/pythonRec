...
with open(drive, "rb") as fileD:
    ...
    while byte:
        ...
        if found >= 0:
            print(f'==== Found MKV at location: {hex(found + (size * offs))} ====')
            drec = True
            rcvd += 1

            # Extract the MKV segment
            mkv_segment = byte[found:]
            ...
            if start_date <= file_creation_date <= end_date:
                ...
                rec_directory_path = os.path.join(script_directory, current_rec_directory, f'{rcvd}.mkv')
                with open(rec_directory_path, 'wb') as fileN:
                    fileN.write(mkv_segment)
                    while drec:
                        byte = fileD.read(size)
                        if byte.startswith(b'\x1A\x45\xDF\xA3'):  # Check for the start of another MKV segment
                            fileN.write(byte)
                            print(f'==== Wrote MKV to location: {rec_directory_path} ====\n')
                            drec = False
                        else:
                            fileN.write(byte)
