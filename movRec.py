...
with open(drive, "rb") as fileD:
    ...
    while byte:
        found = byte.find(b'moov')
        if found >= 0:
            print(f'==== Found MOV at location: {hex(found + (size * offs))} ====')
            drec = True
            rcvd += 1

            # Extract the MOV segment
            mov_segment = byte[found:]
            ...
            if start_date <= file_creation_date <= end_date:
                ...
                rec_directory_path = os.path.join(script_directory, current_rec_directory, f'{rcvd}.mov')
                with open(rec_directory_path, 'wb') as fileN:
                    fileN.write(mov_segment)
                    while drec:
                        byte = fileD.read(size)
                        if byte.startswith(b'moov'):  # Check for the start of another MOV segment
                            fileN.write(byte)
                            print(f'==== Wrote MOV to location: {rec_directory_path} ====\n')
                            drec = False
                        else:
                            fileN.write(byte)
